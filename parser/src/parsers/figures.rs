...
pub fn parse(src: String) -> Vec<ListingNode> {
    ...
    match LaTeXParser::parse(Rule::document, &src) {
        Ok(mut pairs) => {
            ...
            loop {
                ...
                match pair {
                    Some(subpair) => match subpair.as_rule() {
                        ...
                        _ => println!("the last: {}", subpair.as_str()),
                    },
                    None => break,
                }
            }
            ...
            //     println!("getting at: {}", subpair.as_str());
            ...
            println!("found {} listings", list.len());
        }
        Err(error) => {
                    ...
                    match environments::parse_name(env_name.as_str()) {
                        Some(name) => {
                            if name.is_listing() {
                                ...
                                println!(
                                    "found {} environment: with caption:  {} --- label: {}",
                                    env_name.as_str(),
                                    caption_literal,
                                    label_literal
                                );
                                ...
                                let l = ListingNode {
                                    _label: label_literal.to_string(),
                                    _value: caption_literal.to_string(),
                                ...
                                println!("env missing: {}", name.value());
