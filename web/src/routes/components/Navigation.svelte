...
<div class="w-full lg:w-6/12 md:w-8/12 m-auto my-12 flex justify-between">
    <div>
        {#if prev}
        <a data-sveltekit-reload href={section === "" ? `/${prev_path}` : `/${chapter.split(':')[1]}/${prev_path}`}>
            <img
    ...
    <div>
        <a data-sveltekit-reload href={section === "" ? "/" : `/${chapter.split(':')[1]}`}>
            <img
                class="inline dark:hidden relative bottom-1 left-1 pointer-events-none"
    ...
    <div>
        {#if next}
        <a data-sveltekit-reload href={section === "" ? `/${next_path}` : `/${chapter.split(':')[1]}/${next_path}`}>
            <img
                class="inline dark:hidden relative bottom-1 left-1 pointer-events-none"
